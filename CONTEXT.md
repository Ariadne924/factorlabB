...
**因子 (Factor)**：
从行情与扩展数据中计算出的时序信号，用于预测未来收益。所有因子遵循统一签名 `(df: pd.DataFrame) -> pd.Series`；输入必须包含 OHLCV 列，可包含资金费率等扩展列。
_Avoid_：指标、信号、特征
...
**数据 Schema**：
用 Pydantic BaseModel 定义的 API 边界校验格式（KlineRaw / TradeRaw / FundingRateRaw）。校验通过后进入 Bronze，随后转换为标准化 Silver DataFrame。
_Avoid_：数据模型、DTO

## Relationships

- **K 线 → 因子**：所有因子以至少包含 OHLCV 的 DataFrame 为输入，产出索引对齐的 pd.Series
