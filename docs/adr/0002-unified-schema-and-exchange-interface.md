...
**决策**：
1. 用 Pydantic BaseModel 定义三套 API 入口校验模型——`KlineRaw`（K 线）、`TradeRaw`（逐笔成交）、`FundingRateRaw`（资金费率）。所有交易所数据在入口处校验，通过后转为 DataFrame 进入 Bronze 层。管道内部的标准化列定义在 `data/columns.py`（`SILVER_COLUMNS`）。
...
4. v2 变更：原始 Schema 命名从 `KlineSchema`/`TradeSchema`/`FundingRateSchema` 改为 `KlineRaw`/`TradeRaw`/`FundingRateRaw`，强调其只做 API 边界校验，不做管道内部标准化
